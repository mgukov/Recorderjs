...
#LIBOPUS_ENCODER_SRC=$(INPUT_DIR)/encoder.js
#LIBOPUS_DECODER_SRC=$(INPUT_DIR)/decoder.js
#LIBOPUS_ENCODER=$(OUTPUT_DIR)/encoder.min.js
#LIBOPUS_DECODER=$(OUTPUT_DIR)/decoder.min.js

LIBOPUS_LIBOPUS=$(OUTPUT_DIR)/libopus.min.js
...
#$(LIBOPUS_ENCODER) $(LIBOPUS_DECODER) test
default: $(LIBOPUS_LIBOPUS) $(RECORDER) 
...
$(LIBOPUS_LIBOPUS): $(LIBOPUS_OBJ) $(LIBSPEEXDSP_OBJ)
	emcc -o $@ $(EMCC_OPTS) -s EXPORTED_FUNCTIONS="[$(DEFAULT_EXPORTS),$(LIBOPUS_ENCODER_EXPORTS),$(LIBOPUS_DECODER_EXPORTS),$(LIBSPEEXDSP_EXPORTS)]" $(LIBOPUS_OBJ) $(LIBSPEEXDSP_OBJ)
